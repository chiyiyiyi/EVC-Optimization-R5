#!/usr/bin/env bash
# Train TrackQueryHead with 48 queries (capacity variant).
# Usage: bash train_m30b_query48.sh [epochs] [warmup_epochs]
set -e
cd "$(dirname "$0")"
...
TRAIN_ROOT="$PWD/log/m30b_query48_m26e3_e${EPOCHS}_seed54"

python train_temporal_memory.py --config configs/evisseg_evuav.yaml --set \
  DATA.root="$DATA_ROOT" \
  TRAIN.seed=54 TRAIN.epochs="$EPOCHS" TRAIN.batch_size=1 \
  ...
  TEMPORAL_MEMORY.temporal_memory_track_query_num_queries=48 \
  TEMPORAL_MEMORY.temporal_memory_track_query_hidden=128 \
  TEMPORAL_MEMORY.temporal_memory_track_query_max_flow=2.0 \
  TEMPORAL_MEMORY.temporal_memory_track_query_loss_weight=0.05 \
